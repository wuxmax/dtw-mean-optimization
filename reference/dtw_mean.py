...
def ssg_example():
    X = np.random.rand(3,20,1) # 3 sample time series, each of length 20 and dimension 1

    X_mean,f = ssg(X,return_f=True)
...
def dtw_example():
    x = np.reshape(np.array([1,4,3,2]),(4,1))   #reshape N-vector to N-by-1 array
    y = np.reshape(np.array([1,4,4,3,2]),(5,1))
    d = dtw(x,y) # compute only the dtw distance
    print('dist = %.2f' %d)
    
    d,p = dtw(x,y,path=True) # compute the dtw distance and the warping path
    W,V = get_warp_val_mat(p)
    print('Warping matrix:\n',W)
    print('Diagonal of valence matrix:\n',V)
